...
public final class TestDatabase {
    ...
    public static TestDatabase create() {
        ...
        jdbi.useHandle(handle -> {
            ...
            handle.execute("CREATE TABLE characters ("
                // GENERATED BY DEFAULT (not ALWAYS) so the many withCharacter() seeds can still insert an
                // explicit id, while createCharacter()'s @GetGeneratedKeys insert (T1's first test of that
                // path) can omit it and receive a server-assigned one.
                ...
                + "appearance VARCHAR(2000) NOT NULL DEFAULT '{}')"); // T1's cosmetic blob (V17)
    ...
    /**
     * The raw {@code appearance} JSON blob a character was created with (T1 / system design §23), read
     * straight from the column — mirrors {@link #equippedTitleOf}. Nothing reads appearance back in
     * production yet, so a test asserts on the stored text directly rather than through a read DAO.
     */
    public String appearanceOf(long characterId) {
        return jdbi.withHandle(handle -> handle
            .select("SELECT appearance FROM characters WHERE id = ?", characterId)
