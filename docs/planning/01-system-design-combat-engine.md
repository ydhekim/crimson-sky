...
**Potions — a third `SkillType`.** `ACTIVE`/`PASSIVE` (§16) gain a sibling, `CONSUMABLE`. A consumable skill restores a resource (`ResourceType`: `HEALTH`, `MANA`, or `STAMINA`) when a player-set percentage threshold is crossed, and depletes a `charges` count on use:
```java
public enum ResourceType { HEALTH, MANA, STAMINA }
```
`Skill` gains `ResourceType restoresResource`, `int thresholdPercent`, `int restoreAmount`, `int charges` — meaningful only when `type() == CONSUMABLE`. This resolves a real question rather than dodging it: HP/Mana/Stamina reset *between* battles (unchanged — no persistent cross-battle HP, per the earlier decision against that direction), but they fluctuate freely *within* one exactly as they always have, so mid-battle healing has genuine purpose without any change to that model.

**Restore amount, decided 2026-07-16: flat, not a roll.** `restoreAmount` is a plain flat quantity — no `min`/`max` range, no stat bonus, no RNG draw. A potion's potency is a property of the potion, not the drinker: "Small Health Potion" restores 100, "Medium" 200, "Large" 300, each its own authored `Skill` instance (content-authoring, same pattern as the three starter `Weapon`s) — sizing up later just means authoring a new instance, never a new field. This also means resolving a potion trigger consumes **no RNG at all**, unlike every other branch of the cascade.

**`charges` is persisted and genuinely consumed, not a per-battle pool like Mana/Stamina.** Unlike weapon durability/pet health (at most one wear point *per battle*, however many times the item acted), a potion's `charges` decrements once for **every actual trigger**, because a long fight can genuinely cross the threshold and heal more than once. It lives on the `Skill` record itself exactly like `Weapon.currentDurability`/`Pet.currentHealth` — `Inventory` is the source of truth (same drift risk and same resolution as §17: `Loadout`'s copy can go stale, so combat cross-references the equipped id against `Inventory` at battle setup), and the post-battle write tallies *how many times* each potion id fired this battle (not just whether it fired) and decrements `charges` by that count, floored at 0, through the same `updateInventory` call §17/§18 already funnel everything else through. At `charges == 0` the potion never triggers again (same gate as `charges > 0` below) — there is no "repair" for a spent potion; replacing it means acquiring a fresh one, the same way a real consumable item works. Restocking is deliberately out of scope for this section, same as it was for §17/§18's other new mechanics before O1 gave them a shop action.
...
**Two different bonus mechanisms on two different tables, deliberately not one shared mechanism.** The daily battle cap is enforced per-character (§19's query is `WHERE character_id = ?`), so its bonus lives on `characters`. Character slots are inherently an account-level resource — slots belong to the account, not any one character — so that lever lives on `accounts`.

- **Daily battle cap:** `characters.bonus_daily_battles INTEGER NOT NULL DEFAULT 0`. Effective cap = `5 + bonus_daily_battles`. `AttackService` checks `COUNT(*) FROM battle_history WHERE character_id = ? AND created_at > startOfToday` (unfiltered by `won` — every attempt counts against the cap, not just wins) against the effective cap *before* resolving the battle — reject early rather than resolve combat and discard it. The rejection needs a real reason code back to the client (not a silent drop), so the UI can distinguish "daily limit reached" from other failure cases — exact packet/response shape deferred to implementation-prompt time, same treatment as every other pending packet detail in this expansion.
- **Character slots — reuses the existing `accounts.max_slots` column, not a new one.** Corrected 2026-07-20 after reading the actual schema: `accounts.max_slots INTEGER DEFAULT 3` already exists (V1 migration) and is already read into `Account.maxSlots()` and sent to the client in `LoginResponse`/`CharacterListResponse` — but `CharacterService.createCharacter`'s enforcement check is a completely independent hardcoded `>= 3` that never reads `max_slots` at all. Adding a second `bonus_character_slots` column on top, as originally planned here, would have created two divergent sources of truth for the same concept (one displayed, one enforced) and left the existing display/enforcement mismatch unfixed. Instead: `CharacterService.createCharacter`'s check becomes `characterDao.getCharacterCount(accountId) >= maxSlots`, where `maxSlots` is `account.maxSlots()` (the handler already has the full `Account` on `connection.account`, no new read needed); "granting a bonus slot" means incrementing `max_slots` itself (`AccountDao.addCharacterSlots`, an atomic `+= delta`, mirroring `AccountDao.addGlobalCurrency`), not adding a bonus on top of a hardcoded floor.

The daily-battle bonus is a **permanent additive bonus**, not a temporary boost — consistent with "count should be extensible" and the permanent-unlock framing already used for skill points (§15). Character slots get the same permanence via the same underlying mechanism (incrementing the one column that already means "how many slots this account has").
...
```
No `accounts` column — `max_slots` already exists and now does double duty as both the displayed and the enforced value.

**Three grant sources, scoped honestly rather than all resolved here:**
- **IAP** — out of scope for this section; purchase/fulfillment is a platform concern (Epic G/Steam). This section only defines the fields/methods an eventual IAP fulfillment path writes into.
...
## 24. M4 UI/UX design system & open questions — decided 2026-07-22

Written during the M4 pre-work pause (prompt (d) of the sequence: font decision → foundation cleanup, prompt 24 → UI foundation refactor, prompt 25 → this note → screen-by-screen work resumes). Covers the two things D1a (`CombatScreen`) and D4 (loadout screen) need to agree on before either is built for real, plus a testing convention for both.

**Shared visual language — one item-category palette, not two.** `CombatVisualFactory` (prompt 23, currently shelved) assigns a placeholder color family per `ActionSource` — Firebrick for weapons, Royal for skills, Forest for pets, Gold for consumables, dark gray for a failed/Burned cast — with a deterministic per-`itemId` variation so distinct items within a category stay visually distinguishable. D4's loadout screen will need to show a player's equipped weapons/skills/pets too, and if it invents its own colors independently, a player learns two unrelated color codes for the same categories instead of one ("red-ish = weapon") that holds everywhere. When D1a is revisited, pull the category→color mapping out of `CombatVisualFactory` into a shared, combat-agnostic class (e.g. `ui/ItemCategoryPalette`) that both `CombatVisualFactory` and the loadout screen consume — `CombatVisualFactory` becomes a thin wrapper adding the "failed" override and the per-turn swatch sizing, not the owner of the palette itself.

**Pure-logic extraction — the same split the ECS layer already uses, applied to screens.** `ActionResolver` was pulled out of `ActionResolutionSystem` specifically so combat decision logic could be JUnit-tested headless, independent of Ashley. Screens that turn a network response into display text/values should follow the same split: a plain class (no Scene2D, no `Stage`) that takes e.g. an `AttackResponse` and produces the log lines and summary strings, unit-tested in `core`'s existing JUnit 5 source set, with the `Screen` itself doing nothing but handing that output to actors. This applies when D1a's `CombatScreen` is rewritten (its current shelved draft inlines this logic directly in `Screen` methods — a gap to fix on revisit, not a pattern to repeat) and to D4's weight-warning check (`equippedWeapon.weight() > character.comfortableWeight()` is a pure boolean worth its own test, not something to only eyeball in Scene2D).

**D4 open question 1 — loadout priority reordering: up/down buttons, not drag-and-drop.** Scene2D's `DragAndDrop` is meaningfully more code to get right (drop-target highlighting, touch-vs-mouse handling, reorder semantics on drop) than reusing the icon-square button pattern the stat +/- adjusters already establish and prompt 25 formalizes as `UiMetrics.ICON_BUTTON_SIZE`/`UiTheme.iconButtonStyle`. Up/down arrows per pouch row keep the interaction vocabulary this client already has rather than introducing a second one for a single screen. Recommendation, not a mandate — revisit if it feels wrong once the screen is actually laid out.

**D4 open question 2 — weight-warning placement: inline, not a blocking dialog.** System design §4.3's "too heavy for your Strength" effect is a reduced-draw-chance tradeoff the player should be free to weigh, not an error to dismiss — the mechanic is meant to be explored (try a heavy weapon, see the cost, decide), so a dialog interrupting every equip attempt would fight the player's own experimentation. Show the warning inline on the row (small icon/color change next to the weight value, same visual register D3 already uses for a failed/Burned cast) instead. Nothing here currently proposes a hard block on saving an over-weight loadout — if that changes, a confirmation dialog at save time would be the right escalation, not at equip time.
