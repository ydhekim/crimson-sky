...
> **Natural next slice, as of 2026-07-16 (O1/O3 done).** **O2 (potions / `SkillType.CONSUMABLE`) is now the natural next slice** — and it was deliberately split out of the O1/O3 pass rather than silently deferred. O1 and O3 are close mirrors of already-shipped patterns (L1–L3's guarded-spend, N2's wear/durability shape), so they carried very little structural risk. O2 does not fit that mould: it needs a self-targeting heal, which `BattleEngine`'s attacker-deals-damage-to-defender shape has no room for today — a new pre-cascade conditional check, a new `ActionSource`, and a different meaning for `ResolvedAction` ("heal" vs "damage"). Bundling it in would have dragged a structurally novel, higher-risk change into an otherwise low-risk prompt. It is a clean follow-up now that the shop it buys into exists. O4 stays blocked on Epic P (quests don't exist yet), but note that nothing in O1/O3 is waiting on it — see O1's close-out.
