...
Priority: P1. Size: S. Status: todo. **Natural next slice once Epic O prices a skill-restoration scroll** — M1–M3 are done, but M4 needs the scroll to exist as a real item (Epic O shop), which it does not yet; deliberately not implemented this pass. *(Depends on M2. Scroll acquisition depends on the shop/quest epics, not yet designed — this story can implement the mechanic against a placeholder scroll count if those land later.)*
...
Priority: P2. Size: S. Status: todo. **Same as M4** — the reset token is an Epic O shop item that does not exist yet, so this stays deferred to the same natural next slice. *(Depends on M2. Shop-gating depends on the shop epic, not yet designed.)*
...
> **Natural next slice, as of 2026-07-16 (N1/N2 done).** O1 is now the most load-bearing story in the backlog: N2 shipped the depletion but not the recovery, so a weapon that reaches 0 durability is currently **permanently** broken — there is no way back in the game at all. Repair is already priced (`5 gold × missingDurability`, §18), it is the first real gold sink, and it closes the loop N2 opened. O1 also unblocks M4/M5, which have been waiting on the scroll/reset-token to exist as shop items. Everything O1 prices against now exists.
