...
| M2 — Combat Engine core | Server-side ECS systems implementing GDD v4 cascading logic + pet check + Result Set, headless-testable | M1 |
| M3 — Matchmaking & persistence | Queue/pairing, `BattleSession`, reward persistence (Gold/Exp/Elo), battle_history migration | M2 |
| M4 — Combat client | `CombatScreen`, Result Set animation/playback, loadout equip UI | M2 |
...
Detailed stories for M2–M5 are in `docs/planning/02-user-stories.md`. System design for M2/M3 is in `docs/planning/01-system-design-combat-engine.md`.
