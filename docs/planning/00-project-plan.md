...
- **No assets yet:** M2/M3 (the entire combat engine, matchmaking, persistence) need zero assets — it's all data-in/data-out ECS logic. M4 (combat screen) uses placeholder rendering: Scene2D `Image` actors backed by solid-color textures (not raw `ShapeRenderer` draws) and `BitmapFont`/`Label` for action text, routed through one `CombatVisualFactory` lookup, animated with Scene2D `Action`s. That keeps the later real-asset swap a data change, not a rewrite of `CombatScreen`. The same no-assets policy was retrofitted onto the already-built screens (MainMenu/Characters/CharacterCreation/Achievements/Settings) in the M4 foundation cleanup (`docs/planning/24-implementation-prompt-m4-foundation-cleanup.md`): their placeholder art, atlases, and `uiskin.json` were stripped so the whole client — not just the not-yet-built `CombatScreen` — runs asset-free.
- **Real art sourcing (decided 2026-07-27):** working with a commissioned/collaborating designer, not a licensed asset pack — a pack search kept surfacing good-looking kits that didn't fit Crimson Sky's actual content (a generic shop's 18 farming/survival items vs. this game's specific weapons/skills/pets), and chasing thematic fit against someone else's demo assets was a dead end. Style: retro pixel art. Character portraits: modular/layered parts composited at runtime, not one drawing per each of `Appearance`'s 96 raw combinations. Sequencing: UI chrome first (touches every screen already built, most stable target), then crests/stat icons, then item icons, then portrait layers last. Full brief for the designer: `docs/planning/44-art-direction-brief.md`. None of this blocks M4 — it's consumed at M5 through the same `CombatVisualFactory` seam already described below.
